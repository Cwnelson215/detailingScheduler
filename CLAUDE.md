...
- **Public edge path (as of 2026-06-10):** Public `detailing.cwnel.com` does **not** hit Traefik directly. DNS is `CNAME → h.b.nel.family → 66.118.47.137`, an **nginx reverse proxy** (separate `nel.family` infra, managed off-box) in front of bulbasaur Traefik. Since bulbasaur ufw denies WAN and only allows `tailscale0`, that proxy reaches Traefik (`100.78.86.4:443`) over Tailscale. **A site-wide 504 = the nginx proxy's upstream is down**, even when the cluster is healthy. Confirm the cluster directly with `curl -k --resolve detailing.cwnel.com:443:100.78.86.4 https://detailing.cwnel.com/health`.
  - **TLS:** the proxy does **SNI passthrough** (nginx `stream` + `ssl_preread`, routed by SNI) — it does **not** terminate TLS, so the browser negotiates directly with Traefik and sees the valid, auto-renewing cert-manager Let's Encrypt cert. There is **no cert to manage on the proxy**. (Earlier, ~2026-06-06 to 06-09, the proxy terminated TLS with a self-signed minica `*.cwnel.com` cert, which caused a browser "Not secure" warning; switching to passthrough fixed it.) If a "Not secure" warning returns, check the edge proxy first — a stray `http{}` `443 ssl` terminator there will shadow the passthrough and present its own cert.
